...
# Keep the same format as your current removed-platform values,
...
🧑 Add the three R2 secrets to your removed-platform project (Settings → Secrets):
   - `R2_ACCOUNT_ID`
   - `R2_ACCESS_KEY_ID`
   - `R2_SECRET_ACCESS_KEY`

🤖 The agent will run the migration script from the removed-platform shell:

```bash
node scripts/migrate-storage-to-r2.mjs
```

The script copies every cover image and uploaded file from removed-platform's storage
...
ADMIN_PASSWORD              = <from removed-platform>
...
🤖 The agent will run these commands to export your removed-platform Postgres data and
import it into Railway's Postgres.

From the removed-platform shell:
```bash
# Export from removed-platform
...
| `SESSION_SECRET` | removed-platform Secrets | Copy as-is |
| `ADMIN_PASSWORD` | removed-platform Secrets | Copy as-is |
| `AUTOMATION_DRAFT_TOKEN` | removed-platform Secrets | Copy as-is; automation URLs don't change |
| `INDEXNOW_KEY` | removed-platform Secrets | Copy as-is |
| `MCP_CONNECTOR_TOKEN` | removed-platform Secrets | Copy as-is |
| `AI_INTEGRATIONS_*` | removed-platform Secrets | Copy all four as-is |
...
The pg_restore from the removed-platform production database stripped FK and PK constraints
from several tables (the removed-platform DB had an older schema predating these constraints).
