...
- robots.txt: generated by the production server and points at the canonical
  `https://www.mapletechie.com` sitemap URLs
- Canonical public site: `https://www.mapletechie.com`. The frontend server
  can permanently redirect the apex and HTTP `www` variant to HTTPS `www`
  when `CANONICAL_REDIRECT_ENABLED=true`; keep that switch off and the old
  origin available until host-by-host migration checks pass.
...
- `SITE_DOMAIN` — Used in sitemap.xml generation (default: `https://www.mapletechie.com`)
