 ...
 *
 *   • removed-platform / GCS  — used when R2_ACCOUNT_ID is NOT set (dev on removed-platform)
...
// ─── GCS / removed-platform backend ─────────────────────────────────────────────────────

const removed-platform_SIDECAR_ENDPOINT = "http://127.0.0.1:1106";

const gcsClient = new Storage({
  credentials: {
    audience: "removed-platform",
    subject_token_type: "access_token",
    token_url: `${removed-platform_SIDECAR_ENDPOINT}/token`,
    type: "external_account",
    credential_source: {
      url: `${removed-platform_SIDECAR_ENDPOINT}/credential`,
...
async function signGcsObjectURL(opts: {
  ...
  const response = await fetch(
    `${removed-platform_SIDECAR_ENDPOINT}/object-storage/signed-object-url`,
    {
      method: "POST",
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to sign GCS URL (${response.status}). Make sure you are running on removed-platform.`,
    );
  }
...
export class ObjectStorageService {
  /** true → use Cloudflare R2;  false → use removed-platform GCS sidecar */
